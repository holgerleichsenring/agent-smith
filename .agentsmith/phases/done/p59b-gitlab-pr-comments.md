...
- [x] `GitLabMrCommentWebhookHandler` handles Note Hook events
- [x] `GitLabMrCommentReplyService` posts replies to MR
- [x] GitLab token verification in `WebhookVerifier` (already existed from p43e)
- [x] Scenario A (new job) works via GitLab MR comment
- [x] Scenario B (dialogue answer) works via GitLab MR comment
- [x] Unit tests for GitLab-specific payload parsing (8 tests)
