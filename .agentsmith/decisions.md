...
- [Architecture] ListByLifecycleStatusAsync defaults to empty on ITicketProvider. p95c ships the GitHub impl (GetAllForRepository with label filter). GitLab/AzDO/Jira keep the default — reconciler becomes a no-op for them until a follow-up phase adds the listing implementations. This is incremental delivery: the recovery infrastructure is in place and exercised for GitHub, other platforms land when their implementations do.
...
- [Architecture] StaleJobDetector + EnqueuedReconciler run unconditionally on every replica in p95c. Leader-election is deferred to p96 (which introduces the lease primitive for the poller anyway). Duplicate detector/reconciler work is cheap because all transitions are atomic per platform — whichever replica wins sees the updated status and the others no-op on their next scan.
- [Scope] Full LifecycleConfig schema (status_labels, reconciler_interval_minutes, heartbeat_ttl_seconds) deferred to a follow-up. p95c uses hardcoded intervals (heartbeat 30s/2min TTL, stale scan 1min, reconcile 10min) that match p95a's implicit defaults. LifecycleConfig + Jira native-mode probing are naturally paired — they land together when someone wants per-project lifecycle label customisation.
...
- [TradeOff] Hosted services (consumer, stale detector, reconciler) are plain classes with RunAsync(ct), not BackgroundService — matches p95a's PipelineQueueConsumer decision. Cli has no IHost. Task.WhenAll in ServerCommand composes them. When/if Cli migrates to IHost, conversion is mechanical.
...
- [Architecture] IEventPoller reads via ITicketProvider.ListByLifecycleStatusAsync(Pending) — same dependency the reconciler uses. p96 ships the GitHub poller only; other platforms follow when their ListByLifecycleStatus implementations land (same incremental pattern as p95c).
...
- [Scope] Leader lease renewal interval 10s, TTL 30s. 3x safety margin matches the p95c heartbeat pattern; if the process GC-pauses or clock-skews for up to 20s we still renew before the TTL hits.

## p98: Docs Catch-Up for p84 + p95a/b/c + p96
...
- [Scope] Single commit `docs: catch up to p84 + p95a/b/c + p96`. Splitting per-page would create intermediate docs states where one page describes the claim flow and the next still says "webhook → ExecutePipelineUseCase direct". Docs are read coherently or not at all.
