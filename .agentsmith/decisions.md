...
- [Implementation] Resolver also synthesizes-on-the-fly when `Pipelines` is empty but legacy `Pipeline` is set, so direct `new ProjectConfig { Pipeline = "fix-bug" }` test setups don't need to call the normalizer manually. Duplicates ~10 lines of normalizer logic, accepted for test ergonomics — production always goes through the loader, the synth-fallback only triggers in test/programmatic contexts.
- [Implementation] Trigger validation at load time: every value in `(Github|Gitlab|Azuredevops|Jira)Trigger.PipelineFromLabel` and every `DefaultPipeline` must reference a name in `Pipelines`. Mismatch throws `ConfigurationException` naming the project, trigger block, and unknown pipeline name. Errors at boot beat mystery routing at runtime — silent route to hardcoded `"fix-bug"` is what motivated this whole phase.
