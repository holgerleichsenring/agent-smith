...
/// emit directory-shaped paths (e.g. "Sample.CICD/") that git_index_add_bypath then
...
public sealed class LocalSourceProviderStagingTests : IDisposable
{
    ...
    {
        Directory.CreateDirectory(Path.Combine(_repoDir, "Sample.CICD"));
        File.WriteAllText(Path.Combine(_repoDir, "Sample.CICD", "config.yaml"), "k: v");
        File.WriteAllText(Path.Combine(_repoDir, "Sample.CICD", "deploy.yaml"), "k: v");

        using var repo = new Repository(_repoDir);
        var act = () => LocalSourceProvider.StageAllChanges(repo);

        act.Should().NotThrow();
        repo.Index.Should().Contain(e => e.Path == "Sample.CICD/config.yaml");
        repo.Index.Should().Contain(e => e.Path == "Sample.CICD/deploy.yaml");
    }
