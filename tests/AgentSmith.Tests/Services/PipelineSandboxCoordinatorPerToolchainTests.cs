...
public sealed class PipelineSandboxCoordinatorPerToolchainTests
{
    ...
    public async Task FiveCsharpContextsOnOneRepo_CreatesOneSandbox()
    {
        _resolverMock.Setup(r => r.ResolveAllAsync(It.IsAny<RepoConnection>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[]
            {
                ...
                new RemoteContextDiscovery("apiclientgen", "src/ClientGenerator", "csharp"),
                new RemoteContextDiscovery("copyview", "src/copyview", "csharp"),
                new RemoteContextDiscovery("dbmigrator", "src/dbmigrator", "csharp"),
                new RemoteContextDiscovery("structvalidator", "src/structvalidator", "csharp"),
            });
    ...
    public async Task FiveCsharpContextsOnOneRepo_RegistersAllFiveContextsInSandboxContexts()
    {
        ...
        contexts[key].Select(d => d.ContextName)
            .Should().BeEquivalentTo("api", "apiclientgen", "copyview", "dbmigrator", "structvalidator");
    }
