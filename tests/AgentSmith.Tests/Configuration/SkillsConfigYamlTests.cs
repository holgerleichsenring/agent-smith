...
public sealed class SkillsConfigYamlTests : IDisposable
{
    ...
    private AgentSmithConfig Load(string yaml)
    {
        ...
        return new YamlConfigurationLoader(new RawConfigMaterializer(new ProjectConfigNormalizer(new ClarificationParkStatusValidator()), new EffectiveTriggerBuilder(), new DeploymentDefaultsApplier(), new ConfigCatalogResolver(), new AgentSmithPaths()), new NoOpSystemEventPublisher())
