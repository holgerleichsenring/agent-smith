...
public sealed class ServerCommandBuildPollersTests
{
    ...
    public void BuildPollers_LoadsYamlConfig_RegistersOnePerTracker()
    {
        ...
        try
        {
            var config = new YamlConfigurationLoader(new RawConfigMaterializer(new ProjectConfigNormalizer(new ClarificationParkStatusValidator()), new EffectiveTriggerBuilder(), new DeploymentDefaultsApplier(), new ConfigCatalogResolver(), new AgentSmithPaths()), new NoOpSystemEventPublisher())
                .LoadConfig(path);
