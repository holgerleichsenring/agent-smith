...
public sealed class GenerateDocsHandlerSandboxTests
{
    ...
    private static (GenerateDocsHandler Handler, Mock<IAgentProviderFactory> Factory) BuildHandler(
        out Func<ISandbox?> capturedSandbox)
    {
        ...
        return (new GenerateDocsHandler(factoryMock.Object, Mock.Of<IProgressReporter>(),
            NullLogger<GenerateDocsHandler>.Instance), factoryMock);
    }

    private static GenerateDocsContext BuildContext(PipelineContext pipeline)
    {
        ...
        return new GenerateDocsContext(repo, changes, "p", new AgentConfig { Type = "claude" }, pipeline);
    }
}
