...
public class StepExecutorTests
{
    ...
    public async Task ExecuteAsync_RunStep_ReturnsResultWithExitCode()
    {
        ...
        var executor = new StepExecutor(runner.Object, NullLogger<StepExecutor>.Instance);
    ...
    [Fact]
    public async Task ExecuteAsync_RunStepWithEmptyCommand_ThrowsArgumentException()
    {
        var runner = new Mock<IProcessRunner>();
        var executor = new StepExecutor(runner.Object, NullLogger<StepExecutor>.Instance);
        var step = new Step(Step.CurrentSchemaVersion, Guid.NewGuid(), StepKind.Run, "");

        var act = () => executor.ExecuteAsync(step, _ => Task.CompletedTask, CancellationToken.None);
    ...
    {
        var executor = new StepExecutor(new ProcessRunner(), NullLogger<StepExecutor>.Instance);
