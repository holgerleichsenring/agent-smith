...
/// <summary>
/// Loads code-map.yaml from the repository root into the pipeline context.
    ...
    : ICommandHandler<LoadCodeMapContext>
{
    ...
    {
        var codeMapPath = Path.Combine(context.Repository.LocalPath, CodeMapFileName);

        if (!File.Exists(codeMapPath))
        {
            logger.LogInformation("No {File} found, continuing without code map", CodeMapFileName);
        ...
        logger.LogInformation("Loaded {File} ({Chars} chars)", CodeMapFileName, content.Length);
