...
public static partial class CommandNames
{
    ...
    public static readonly IReadOnlyDictionary<string, string> RetiredCommands =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["BootstrapProjectCommand"] = "BootstrapProject was retired in p0131b. The init-project pipeline now produces .agentsmith/context.yaml + principles.md via a SkillRound dispatch (csharp/node/python/generic-bootstrap); other pipelines should rely on the BootstrapGate added in p0130a to fail-fast on missing files. Remove the step from your custom preset.",
            ["LoadCodeMapCommand"] = "LoadCodeMap was retired in p0131b together with the code-map.yaml artifact. ProjectMap (populated by AnalyzeCode) is now the single source of truth for module/test-project structure; ContextKeys.RepoCodeMaps is still emitted as a per-repo free-form text rendering for prompt-builders. Remove the step from your custom preset.",
