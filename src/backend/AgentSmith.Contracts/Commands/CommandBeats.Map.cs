...
{
    private static readonly Dictionary<string, RunBeat> Beats = BuildBeats();

    private static Dictionary<string, RunBeat> BuildBeats()
    {
        var beats = new Dictionary<string, RunBeat>(StringComparer.Ordinal)
        {
        ...
        foreach (var (command, beat) in OutcomeBeats()) beats[command] = beat;
        return beats;
    }
}
