...
internal sealed class JobLoop(
    IRedisJobBus bus, ToleratedStepReader steps, IStepExecutor executor,
    IStepInFlightMarker heartbeat, ILogger<JobLoop> logger, string? runId = null)
{
    ...
    public async Task<int> RunAsync(string jobId, CancellationToken cancellationToken)
    {
        ...
        {
            var step = await steps.NextAsync(jobId, IdlePollTimeout, cancellationToken);
            if (step is null)
            {
