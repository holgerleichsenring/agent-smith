...
/// <summary>
/// Implements <see cref="IRegistryAuthStager"/> as a Scout-role, read-only
/// agentic call: the model explores the repo, works out which package manager
/// references each uncovered host, and emits that manager's GLOBAL auth-config
/// file(s) with <c>__AS_TOKEN_&lt;host&gt;__</c> placeholders. The prompt carries
/// only the hosts and the placeholder convention — never a token — so the secret
/// cannot enter the LLM prompt, response, or history.
/// </summary>
    ...
    ILogger<RegistryAuthStager> logger) : IRegistryAuthStager
{
    ...
    private static string BuildSystemPrompt() => $$"""
        ...
        You are given one or more registry HOSTS. For each host, inspect the repository's
        config and manifest files (read-only tools) to determine which package manager
        (cargo, go, maven, pip, gradle, composer, gem, …) references that host, then emit
        ...
        - Emit ONLY user/global-scope absolute paths (for example under /root or the
          manager's user config dir). Never write inside the repo checkout.
    ...
    private static string BuildUserPrompt(string repoRoot, IReadOnlyList<UncoveredRegistry> uncovered)
    {
        ...
        sb.AppendLine("Registry hosts to stage (with a repo file that references each):");
        foreach (var u in uncovered)
            sb.AppendLine($"- {u.Registry.Host}  (referenced by {u.HintPath})");
        sb.AppendLine();
        sb.AppendLine("Inspect the relevant manifests, then emit the global auth-config file(s).");
        return sb.ToString();
    }
}
