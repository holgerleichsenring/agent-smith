...
/// has no configured registry fails the whole file LOUDLY rather than writing
/// an empty or half-substituted auth config that silently breaks restore.
/// </summary>
    ...
    public TokenSubstitutionResult Substitute(string content, IReadOnlyList<RegistryConfig> registries)
    {
        ...
        if (unmatched.Count > 0)
            return TokenSubstitutionResult.Fail(
                $"placeholder host(s) with no configured registry token: [{string.Join(", ", unmatched)}]");
