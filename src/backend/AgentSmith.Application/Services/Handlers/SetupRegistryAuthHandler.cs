    ...
    : ICommandHandler<SetupRegistryAuthContext>
{
        ...
        SetupRegistryAuthContext context, CancellationToken cancellationToken)
    {
        ...
        // run's resolved agent; read it lazily so the fast-path-only path — and unit
        // tests without a resolved pipeline — never touch it.
        ...
        string repoKey, ISandbox sandbox, Func<AgentConfig> agentFactory, CancellationToken ct)
    {
        ...
        // cover but that another ecosystem's manifest references, hand the leftover
        // set to the LLM fallback (token substituted host-side, never sent to the LLM).
